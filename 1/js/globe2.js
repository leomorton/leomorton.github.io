          ...
          $(document).on('click touchstart', function () {
            ...
            if (stage_0==false&&isoverbutton==false) {
                ...
                var ic1 = raycaster.intersectObjects(model.children);
                    if (ic1.length > 0) {
                        if(ic1[0].object.name=="Christmas_hoerarea"){bga.pause();window.parent.postMessage("btnBaubleEntry",'*');}
                        if(ic1[0].object.name=="Pavillion_hoverarea"){bga.pause();window.parent.postMessage("Pavillion_click",'*');}
                        if(ic1[0].object.name=="Pavillion_click"){bga.pause();window.parent.postMessage("Pavillion_click",'*');}
                        if(ic1[0].object.name=="Ice_rink_hoverarea"){bga.pause();window.parent.postMessage("iceRinkBuilding_click",'*');}
                        if(ic1[0].object.name=="Santas_grotto_hoverarea"){bga.pause();window.parent.postMessage("santasGrotto_click",'*');}
                        if(ic1[0].object.name=="Food_stall_hoverarea"){bga.pause();window.parent.postMessage("btnRecipes",'*');}
                        if(ic1[0].object.name=="Games_stall_hoverarea"){bga.pause();window.parent.postMessage("btnIframe_stack",'*');}
                        if(ic1[0].object.name=="Craft_Stall_hoverarea"){bga.pause();window.parent.postMessage("marketStall_Crafts_click",'*');}
                        if(ic1[0].object.name=="Cracker_stall_hoverarea"){bga.pause();window.parent.postMessage("btnJokes",'*');}
                        if(ic1[0].object.name=="Bandstand_hoverarea"){bga.pause();window.parent.postMessage("btnSpotify",'*');}
                        if(ic1[0].object.name=="Post_box_hoverarea"){bga.pause();window.parent.postMessage("postBox_click",'*');}
                    }
